  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium">Import From Resume</h3>
      ...
      >
        ...
        {isProcessing ? (
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            <div
              className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors duration-200 ${isDragging ? "bg-primary/10" : "bg-muted"}`}
            >
              <CloudUpload
                className={`h-6 w-6 transition-colors duration-200 ${isDragging ? "text-primary" : "text-muted-foreground"}`}
              />
            </div>
            <div>
      ...
      {!error && !uploadedFileName && (
        <p className="text-xs text-muted-foreground">
          Upload your resume PDF or DOCX and AI will automatically fill all
          portfolio fields for you.
        </p>
      )}
    </div>
  );
