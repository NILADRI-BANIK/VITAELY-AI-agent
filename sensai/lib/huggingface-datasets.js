...
async function fetchHuggingFace(baseUrl, endpoint, params = {}) {
  ...
  if (res.status === 401) {
    throw new Error("HuggingFace authentication failed. Check HUGGINGFACE_API_KEY.");
  }
  if (res.status === 403) {
    throw new Error("HuggingFace access forbidden. Verify your API key permissions.");
  }
  if (res.status === 429) {
...
export async function getDatasetParquetFiles(datasetId, config = null, split = null) {
  const params = { dataset: datasetId };
  if (config) params.config = config;
  ...
  const data = await fetchHuggingFace(
    DATASETS_URL,
    "/parquet",
    { dataset: datasetId, config, split }
  );
  return data?.parquet_files ?? [];
}

export async function getDatasetPreview(datasetId, config = null, split = "train", options = {}) {
  ...
  const data = await fetchHuggingFace(
    DATASETS_URL,
    "/rows",
    { dataset: datasetId, config, split, offset, length }
  );
  return {
    rows: data?.rows ?? [],
