...
export async function getPaperRecommendations(paperId, options = {}) {
  ...
  let res;
  try {
    res = await fetch(url.toString(), { headers: getHeaders() });
  } catch (error) {
...
export async function fetchSemanticScholarPaper(doiOrTitle) {
  try {
    if (doiOrTitle.startsWith("10.")) {
      return await getPaperByDOI(doiOrTitle);
    }
    const result = await searchPapers(doiOrTitle, { limit: 1 });
...
export async function fetchSemanticScholarTopics(options = {}) {
  const { search = "", limit = 20 } = options;
  if (!search?.trim?.() && !search) return [];
  const data = await searchPapers(search, { limit, fields: "fieldsOfStudy" });
  const seen = new Map();
  (data?.data ?? []).forEach((p) => {
    (p.fieldsOfStudy ?? []).forEach((f) => {
      if (!seen.has(f)) seen.set(f, { topicId: f, name: f, paperCount: 0 });
      seen.get(f).paperCount += 1;
    });
  });
  return Array.from(seen.values());
}

export async function fetchSemanticScholarPapers(options = {}) {
  const { query = "", limit = 10 } = options;
  if (!String(query).trim()) return { data: [], total: 0 };
  try {
