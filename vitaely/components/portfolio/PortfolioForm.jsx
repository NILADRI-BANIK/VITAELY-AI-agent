...
// ─────────────────────────────────────────────
// HELPER — split a "Month Year" / "Year" / "" date string
// (as emitted by lib/portfolio/parseResume.js) into the
// { month, year } shape the DateSelect UI expects.
// Accepts:
//   "Jan 2022" / "January 2022" -> { month: "January", year: "2022" }
//   "2022"                      -> { month: "", year: "2022" }
//   "" / null / undefined       -> { month: "", year: "" }
// Falls back gracefully on anything it can't parse.
// ─────────────────────────────────────────────
const MONTH_ALIASES = {
  jan: "January",
  feb: "February",
...
function splitDateString(dateStr) {
  ...
  // Pull out whatever's left as the month token
  const monthToken = trimmed.replace(year, "").trim().replace(/[,\s]+$/, "");

  if (!monthToken) {
    return { month: "", year };
  }

  // Exact match against full month names
  const exact = MONTHS.find(
    (m) => m.toLowerCase() === monthToken.toLowerCase(),
  );
  if (exact) return { month: exact, year };

  // Match against common 3-4 letter abbreviations
  const alias = MONTH_ALIASES[monthToken.toLowerCase()];
  if (alias) return { month: alias, year };

  return { month: "", year };
}

// Applies splitDateString to startDate/endDate on a single item,
// returning a new object with startMonth/startYear/endMonth/endYear added.
function withSplitDates(item) {
  const start = splitDateString(item?.startDate);
  const end = splitDateString(item?.endDate);
  return {
    ...item,
...
export default function PortfolioForm({
  ...
  // skillGroups holds the real categorized result: [{ category, skills: [] }]
  // Populated by Auto Categorize, by loading an existing portfolio, or by
  // resume parsing (all of which already produce this shape upstream).
  // Cleared whenever the raw textarea is hand-edited, so stale grouped
  // chips can't drift out of sync with the free-text list.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Skills come back from the DB as [{category, skills, proficiency}]
        // (see actions/portfolio.js createPortfolio/updatePortfolio, which
        // stores whatever buildFormData sends). Populate both the flat
        // textarea (for editing) and skillGroups (for the grouped display).
        if (Array.isArray(portfolio.skills) && portfolio.skills.length > 0) {
          const looksGrouped = portfolio.skills.every(
            (s) => s && typeof s === "object" && Array.isArray(s.skills),
          );
          if (looksGrouped) {
            setSkillGroups(portfolio.skills);
            setSkillsRaw(
              portfolio.skills.flatMap((g) => g.skills || []).join(", "),
            );
          } else {
  ...
  const handleParsed = (data) => {
    ...
    if (Array.isArray(data.skills) && data.skills.length > 0) {
      // parseResume.js emits [{category, skills, proficiency}] — keep the
      // grouping for skillGroups, and also flatten into skillsRaw so the
      // free-text textarea stays in sync for manual editing.
    ...
    // experience/education/projects come back with flat startDate/endDate
    // strings (e.g. "Jan 2022") from parseResume.js, but the form fields
    // and DateSelect UI expect split startMonth/startYear/endMonth/endYear.
    // withSplitDates() bridges that gap so dates actually populate.
    setExperience(
  ...
  const handleCategorizeSkills = async () => {
    ...
    try {
      ...
      // Keep the real grouped result instead of flattening it away —
      // this is what actually renders in the grouped Skills UI below.
      setSkillGroups(Array.isArray(categorized) ? categorized : []);
  ...
  const buildFormData = () => ({
    ...
    hackerrank,
    codeforces,
    // Send the grouped shape ({category, skills}) whenever we have one —
    // this matches what the DB/generatePortfolioData already expect.
    // Fall back to a single "General" group built from the raw textarea
    // if the user never ran Auto Categorize.
    skills:
        ...
        <CardContent className="pt-6 space-y-3">
          ...
          <Textarea
            ...
            onChange={(e) => {
              ...
              // Raw list no longer matches whatever grouping we had —
              // clear it so stale category chips can't drift out of sync.
              if (skillGroups.length > 0) setSkillGroups([]);
            }}
          ...
          {/* Grouped display — only shown once we have a real categorized result */}
          {skillGroups.length > 0 && (
            <div className="space-y-3 pt-2">
              {skillGroups.map((group, gi) => (
                <div key={`${group.category}-${gi}`} className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(group.skills || []).map((skill, si) => (
                      <span
                        key={`${skill}-${si}`}
                        className="inline-flex items-center rounded-full border bg-primary/5 px-3 py-1 text-xs font-medium text-primary"
                      >
                        {skill}
                      </span>
                    ))}
